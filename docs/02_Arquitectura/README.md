...
*   **Caso de Uso (Cancelación por Emergencia - RF-20):** Cuando el administrador fuerza una sala a estado `MANTENIMIENTO`, el `Showtime Service` cancela las funciones locales y dispara el evento `ShowtimeCancelledEvent`. El `Seat Service` reacciona invalidando los tickets (`CANCELLED`). Otros servicios (o el mismo `Seat Service`) reaccionan generando un Cupón de Cortesía (Bypass de pasarela) en la base de datos interna y disparando el proceso de reembolso (Refund) mediante API asíncrona hacia Culqi, finalizando con un correo de disculpas al usuario.
